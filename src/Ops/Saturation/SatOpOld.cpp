...
REGISTER_OP("SatOp")
    ...
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
      ...
      TF_RETURN_IF_ERROR(c->WithRank(c->input(1), 2, &p_shape));
      ...
      TF_RETURN_IF_ERROR(c->WithRank(c->input(6), 0, &dt_shape));
      shape_inference::ShapeHandle h_shape;
      TF_RETURN_IF_ERROR(c->WithRank(c->input(7), 0, &h_shape));
...
class SatOpOp : public OpKernel {
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Tensor& dt = context->input(6);
    ...
    // TODO:
    forward(sat_tensor, s0_tensor, p_tensor, permi_tensor, poro_tensor,
            qw_tensor, qo_tensor, *dt_tensor, *h_tensor, nz, nx);
  }
};
...
class SatOpGradOp : public OpKernel {
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Tensor& dt = context->input(8);
    const Tensor& h = context->input(9);
    ...
    Tensor* grad_p = NULL;
    OP_REQUIRES_OK(context, context->allocate_output(1, grad_p_shape, &grad_p));
    ...
    Tensor* grad_dt = NULL;
    OP_REQUIRES_OK(context,
                   context->allocate_output(6, grad_dt_shape, &grad_dt));
    Tensor* grad_h = NULL;
    OP_REQUIRES_OK(context, context->allocate_output(7, grad_h_shape, &grad_h));
