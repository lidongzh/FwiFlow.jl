...
__global__ void moduliInit(float *d_Cp, float *d_Cs, float *d_Den, float *d_Lambda, float *d_Mu, \
    ...
    if (gidx < nx && gidy < ny) {
        ...
        d_Lambda[offset] = d_Den[offset] * (powf(d_Cp[offset], 2) - 2 * powf(d_Cs[offset], 2));
...
__global__ void aveBycInit(float *d_Den, float *d_ave_Byc_a, float *d_ave_Byc_b, int nx, int ny){
    ...
    int gidy = threadIdx.y + blockDim.y * blockIdx.y;
    if (gidx>=2 && gidx<=nx-3 && gidy>=2 && gidy<=ny-3) {
        d_ave_Byc_a(gidx,gidy) = 0.5 * (1.0/d_Den(gidx+1,gidy) + 1.0/d_Den(gidx,gidy));
        d_ave_Byc_b(gidx,gidy) = 0.5 * (1.0/d_Den(gidx,gidy-1) + 1.0/d_Den(gidx,gidy));
    }
    else {
        return;
    }
...
__global__ void gpuMinus(float *d_out, float *d_in1, float *d_in2, int nx, int ny){
    ...
    if (idx<nx && idy<ny && idx>0) {
        d_out[(idy)*(nx)+(idx)] = d_in1[(idy)*(nx)+(idx)] - d_in2[(idy)*(nx)+(idx)];
    } 
...
void cpmlInit(float *K, float *a, float *b, float *K_half, \
    ...
    for(int i=0; i<N; i++) {
        ...
        if(depth_in_pml >= 0.0) {
            ...
            damp[i] = d0_h * (c1*depth_normalized +  c2*pow(depth_normalized, NPOWER) \
                + c3*pow(depth_normalized, 4));
        ...
        if(depth_in_pml >= 0.0) {
        depth_normalized = depth_in_pml / thickness_PML;
            damp_half[i] = d0_h * (c1*depth_normalized +  c2*pow(depth_normalized, NPOWER) \
                + c3*pow(depth_normalized, 4));
        ...
        // right edge
        // depth_in_pml = (nPml-N+i+1.0)*dh;
        depth_in_pml = (nPml-N+i+1.0+0.5)*dh;
        if(depth_in_pml >= 0.0) {
            depth_normalized = depth_in_pml/thickness_PML;
            damp[i] = d0_h * (c1*depth_normalized +  c2*pow(depth_normalized, NPOWER) \
                    + c3*pow(depth_normalized, 4));
        ...
        if(fabs(damp_half[i]) > 1.0e-6){
            a_half[i] = damp_half[i] * (b_half[i] - 1.0) / (K_half[i] * (damp_half[i] \
                + K_half[i] * alpha_half[i]));
        }
    }
    free(damp);
    free(damp_half);
...
//     float depth_normalized = 0.0;
//     float thickness_PML = 0.0;
//     float originleft = 0.0;
//     float originright = 0.0;
//     const float PI = 3.141592653589793238462643383279502884197169;
//     const float K_MAX_PML = 1.0;
//     const float ALPHA_MAX_PML = 2.0*PI*(f0/2.0);
//     const float NPOWER = 2.0;

//     thickness_PML = (nPml-1) * dh; // changed here 
//     d0_h = - (NPOWER + 1) * CpAve * log(Rcoef) / (2.0 * thickness_PML);
//     damp = (float*)malloc(N*sizeof(float));
//     damp_half = (float*)malloc(N*sizeof(float));
//     alpha = (float*)malloc(N*sizeof(float));
...
//         // left edge
//         hval = dh * float(i);
//         depth_in_pml = originleft - hval;
//         if(depth_in_pml >= 0.0) {
//             depth_normalized = depth_in_pml / thickness_PML;
//             damp[i] = d0_h * powf(depth_normalized, NPOWER);
//             K[i] = 1.0 + (K_MAX_PML - 1.0) * powf(depth_normalized, NPOWER);
//             alpha[i] = ALPHA_MAX_PML * (1.0 - depth_normalized) + 0.1 * ALPHA_MAX_PML;
//         }

//         // half the grid points
//         depth_in_pml = originleft - (hval + dh/2.0);
//         if(depth_in_pml >= 0.0) {
//             depth_normalized = depth_in_pml / thickness_PML;
//             damp_half[i] = d0_h * powf(depth_normalized, NPOWER);
//             K_half[i] = 1.0 + (K_MAX_PML - 1.0) * powf(depth_normalized, NPOWER);
//             alpha_half[i] = ALPHA_MAX_PML * (1.0 - depth_normalized) + 0.1 * ALPHA_MAX_PML;
//         }


//         // right edge
//         depth_in_pml = hval - originright;
//         if(depth_in_pml >= 0.0) {
//             depth_normalized = depth_in_pml/thickness_PML;
//             damp[i] = d0_h * powf(depth_normalized, NPOWER);
//             K[i] = 1.0 + (K_MAX_PML - 1.0) * powf(depth_normalized, NPOWER);
...
//         depth_in_pml = hval + dh/2.0 - originright;
//         if(depth_in_pml >= 0.0) {
//             depth_normalized = depth_in_pml / thickness_PML;
//             damp_half[i] = d0_h * powf(depth_normalized, NPOWER);
//             K_half[i] = 1.0 + (K_MAX_PML - 1.0) * powf(depth_normalized, NPOWER);
...
__global__ void from_bnd(float *d_field, float *d_bnd, int nz, int nx, int nzBnd, \
  int nxBnd, int len_Bnd_vec, int nLayerStore, int indT, int nPml, int nPad, int nSteps) {
    ...
    if(idxBnd>=0 && idxBnd<=nLayerStore*nzBnd-1) {
        ...
        d_bnd(idxBnd,indT) = d_field((iRow+nPml),(jCol+nPml));
    }
        ...
        d_bnd(idxBnd,indT) = d_field((iRow+nPml),(nx-nPml-jCol-1));
    }
    else if(idxBnd>=2*nLayerStore*nzBnd && idxBnd<=nLayerStore*(2*nzBnd+nxBnd)-1) {
        iRow = (idxBnd - 2*nLayerStore*nzBnd)/nxBnd;
        jCol = (idxBnd - 2*nLayerStore*nzBnd) - iRow*nxBnd;
        d_bnd(idxBnd,indT) = d_field((iRow+nPml),(jCol+nPml));
    }
    else if(idxBnd>=nLayerStore*(2*nzBnd+nxBnd) && idxBnd<=2*nLayerStore*(nzBnd+nxBnd)-1) {
        iRow = (idxBnd - nLayerStore*(2*nzBnd+nxBnd))/nxBnd;
        jCol = (idxBnd - nLayerStore*(2*nzBnd+nxBnd)) - iRow*nxBnd;
        d_bnd(idxBnd,indT) = d_field((nz-nPml-nPad-iRow-1),(jCol+nPml));
    }
    else {
        return;
    }
...
__global__ void to_bnd(float *d_field, float *d_bnd, int nz, int nx, int nzBnd, \
  int nxBnd, int len_Bnd_vec, int nLayerStore, int indT, int nPml, int nPad, int nSteps) {
    ...
    if(idxBnd>=0 && idxBnd<=nLayerStore*nzBnd-1) {
        jCol = idxBnd/nzBnd;
        iRow = idxBnd - jCol*nzBnd;
        d_field((iRow+nPml),(jCol+nPml)) = d_bnd(idxBnd,indT);
    }
    else if(idxBnd>=nLayerStore*nzBnd && idxBnd<=2*nLayerStore*nzBnd-1){
        jCol = (idxBnd-nLayerStore*nzBnd)/nzBnd;
        iRow = (idxBnd-nLayerStore*nzBnd) - jCol*nzBnd;
        d_field((iRow+nPml),(nx-nPml-jCol-1)) = d_bnd(idxBnd,indT);
    }
    else if(idxBnd>=2*nLayerStore*nzBnd && idxBnd<=nLayerStore*(2*nzBnd+nxBnd)-1) {
        iRow = (idxBnd - 2*nLayerStore*nzBnd)/nxBnd;
        jCol = (idxBnd - 2*nLayerStore*nzBnd) - iRow*nxBnd;
        ...
        iRow = (idxBnd - nLayerStore*(2*nzBnd+nxBnd))/nxBnd;
        jCol = (idxBnd - nLayerStore*(2*nzBnd+nxBnd)) - iRow*nxBnd;
        d_field((nz-nPml-nPad-iRow-1),(jCol+nPml)) = d_bnd(idxBnd,indT);
    }
    else {
        return;
    }
...
__global__ void cuda_spectrum_update(int nf, int nrec, cuFloatComplex *d_data_obs_F, \
    ...
    int tid = threadIdx.x; // one thread handles s receivers (with 512 as the interval)
    ...
    for(int s = 0; s < (nrec+Block_Size-1)/Block_Size; s++) {
        ...
        if (idr >= 0 && idr < nrec && idf >= 0 && idf < nf) {
            ...
            sh_nominator_F[tid] = cuCaddf(sh_nominator_F[tid], cuCmulf(cuConjf(c_cal), c_obs));
            sh_denominator_F[tid] = cuCaddf(sh_denominator_F[tid], cuCmulf(cuConjf(c_cal), c_cal));
        }
    } 
    __syncthreads();
    ...
    for(int s = blockDim.x/2; s >= 1; s /= 2) {
        if (tid < s) {
            sh_nominator_F[tid] = cuCaddf(sh_nominator_F[tid], sh_nominator_F[tid + s]);
            sh_denominator_F[tid] = cuCaddf(sh_denominator_F[tid], sh_denominator_F[tid + s]);
        }
        __syncthreads();
    }
    if (tid == 0) {
        sh_denominator_F[0].x += lambda;
        // printf("nomi = %f, deno = %f\n", cuCabsf(sh_nominator_F[0]), cuCabsf(sh_denominator_F[0]));
...
__global__ void cuda_find_absmax(int n, float *data, float *maxval) {
    ...
    for(int s = 0; s < (n+Block_Size-1)/Block_Size; s++) {
        ...
        if (ip >= 0 && ip < n) {
            if (fabs(data[ip]) > fabs(sh_data[tid])) sh_data[tid] = \
                fabs(data[ip]);
        }
    }
    __syncthreads();
    ...
    for(int s = blockDim.x/2; s >= 1; s /= 2) {
        if (tid < s) {
            sh_data[tid] = (sh_data[tid]>=sh_data[tid+s])?sh_data[tid]:sh_data[tid+s];
        }
        __syncthreads();
    }
...
void bp_filter1d(int nSteps, float dt, int nrec, float *d_data, float *filter) {
    ...
    cuda_embed_crop<<<blocks, threads>>>(nSteps, nrec, d_data, nSteps_pad, \
        nrec, d_data_pad, true);

    // CHECK(cudaMemcpy(h_test, d_data_pad, nSteps * sizeof(float), cudaMemcpyDeviceToHost));
    // displayArray("h_test", h_test, nSteps_pad, 1);
    ...
    cuda_bp_filter1d<<<blocks,threads>>>(nSteps_pad, dt, nrec, d_data_F, f0, f1, f2, f3);
    ...
    // crop data
    cuda_embed_crop<<<blocks, threads>>>(nSteps, nrec, d_data, nSteps_pad, \
        nrec, d_data_pad, false);

    // normalization (in the padded fft, the length is nSteps_pad)
    cuda_normalize<<<blocks,threads>>>(nSteps, nrec, d_data, 1/float(nSteps_pad));
...
float source_update(int nSteps, float dt, int nrec, float *d_data_obs, float *d_data_cal, \
    ...
    CHECK(cudaMalloc((void**)&d_data_obs_pad, nSteps_pad * nrec * sizeof(float)));
    CHECK(cudaMalloc((void**)&d_data_cal_pad, nSteps_pad * nrec * sizeof(float)));
    ...
    intialArrayGPU<<<(nSteps_pad+31)/32, 32>>>(d_source_pad, nSteps_pad, 1, 0.0);
    ...
    cuda_window<<<blocks_pad, threads>>>(nSteps_pad, nrec, dt, 0.01, d_data_obs_pad);
    cuda_window<<<blocks_pad, threads>>>(nSteps_pad, nrec, dt, 0.01, d_data_cal_pad);

    cuda_embed_crop<<<(nSteps_pad+31)/32, 32>>>(nSteps, 1, d_source, nSteps_pad, \
        1, d_source_pad, true);
    ...
    cufftPlan1d(&plan_f, nSteps_pad, CUFFT_R2C, nrec);
    cufftExecR2C(plan_f, d_data_obs_pad, d_data_obs_F); // forward FFT of observed data
    ...
    // cuda_bp_filter1d<<<blocks,threads>>>(nSteps_pad, dt, nrec, d_data_F, f0, f1, f2, f3);
    cuda_spectrum_update<<<nfft, 512>>>(nfft, nrec, d_data_obs_F, d_data_cal_F, d_source_F, d_coef);
    ...
    // normalization (in the padded fft, the length is nSteps_pad)
    // printf("amp = %f\n", amp_ratio);
    cuda_normalize<<<blocks,threads>>>(nSteps, nrec, d_data_cal, 1.0f/float(nSteps_pad));
    cuda_normalize<<<(nSteps+31)/32, 32>>>(nSteps, 1, d_source, 1.0f/float(nSteps_pad));
    ...
    // cuda_normalize<<<(nSteps+31)/32, 32>>>(nSteps, 1, d_source, amp_ratio/float(nSteps_pad));

    // // update amplitude
    // cuda_find_absmax<<<1, 512>>>(nSteps*nrec, d_data_obs, d_obs_maxval);
    // cuda_find_absmax<<<1, 512>>>(nSteps*nrec, d_data_cal, d_cal_maxval);
    // CHECK(cudaMemcpy(obs_maxval, d_obs_maxval, sizeof(float), cudaMemcpyDeviceToHost));
    // CHECK(cudaMemcpy(cal_maxval, d_cal_maxval, sizeof(float), cudaMemcpyDeviceToHost));
    // cuda_normalize<<<blocks,threads>>>(nSteps, nrec, d_data_cal, 1.0/amp_ratio);
...
void source_update_adj(int nSteps, float dt, int nrec, float *d_data, \
    ...
    // normalization (in the padded fft, the length is nSteps_pad)
    // printf("amp_adj = %f\n", amp_ratio);
    cuda_normalize<<<blocks,threads>>>(nSteps, nrec, d_data, amp_ratio/float(nSteps_pad));
...
float amp_ratio_comp(int n, float *d_data_obs, float *d_data_cal) {
    ...
    float ratio = obs_maxval[0]/cal_maxval[0];
