...
Bnd::Bnd(const Parameter &para) {
	...
	if (withAdj_) {
		...
		len_Bnd_vec_ = 2 * (nLayerStore_*nzBnd_ + nLayerStore_*nxBnd_); // store n layers

		// allocate the boundary vector in the device
		CHECK(cudaMalloc((void**)&d_Bnd_szz, len_Bnd_vec_*nSteps_*sizeof(float)));
		if (isAc_ != true) {
			CHECK(cudaMalloc((void**)&d_Bnd_sxz, len_Bnd_vec_*nSteps_*sizeof(float)));
			CHECK(cudaMalloc((void**)&d_Bnd_sxx, len_Bnd_vec_*nSteps_*sizeof(float)));
		}
		CHECK(cudaMalloc((void**)&d_Bnd_vz, len_Bnd_vec_*nSteps_*sizeof(float)));
		CHECK(cudaMalloc((void**)&d_Bnd_vx, len_Bnd_vec_*nSteps_*sizeof(float)));
	}
...
void Bnd::field_from_bnd(float *d_szz, float *d_sxz, float *d_sxx, \
		float *d_vz, float *d_vx, int indT) {
	from_bnd<<<(len_Bnd_vec_+31)/32,32>>>(d_szz, d_Bnd_szz, nz_, nx_, nzBnd_, \
  		nxBnd_, len_Bnd_vec_, nLayerStore_, indT, nPml_, nPad_, nSteps_);
...
void Bnd::field_to_bnd(float *d_szz, float *d_sxz, float *d_sxx, \
		float *d_vz, float *d_vx, int indT, bool if_stress) {
	if (if_stress) {
