// Coalesced read and shared memory
...
__global__ void update_velocity_shco(float *d_vz, float *d_vx, float *d_szz, \
 ...
 //  int gidx = blockIdx.y*blockDim.y + threadIdx.y + zerowidth;
  ...
  if(lidz < 4){
  	sh_szz(lidz-2,lidx) = (gidz >= 2)?d_szz(gidz-2,gidx):0.0;
  	sh_sxz(lidz-2,lidx) = (gidz >= 2)?d_sxz(gidz-2,gidx):0.0;
  }

  if(lidx < 4){
    sh_sxz(lidz,lidx-2) = (gidx >= 2)?d_sxz(gidz,gidx-2):0.0;
    sh_sxx(lidz,lidx-2) = (gidx >= 2)?d_sxx(gidz,gidx-2):0.0;
  }

  // if(gidz < 2){
  //   sh_szz(lidz-2,lidx) = 0.0;
  //   sh_sxz(lidz-2,lidx) = 0.0;
  // }else if(lidz < 4){
  ...
  // if(gidx < 2){
  //   sh_sxz(lidz,lidx-2) = 0.0;
  //   sh_sxx(lidz,lidx-2) = 0.0;
  // }else if(lidx < 4){
