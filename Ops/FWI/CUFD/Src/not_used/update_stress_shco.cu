...
__global__ void update_stress_shco(float *d_vz, float *d_vx, float *d_szz, \
  ...
  sh_vx(lidz,lidx) = d_vx(gidz,gidx);
 	...
 	if(lidz < 4){
  	sh_vz(lidz-2,lidx) = (gidz >= 2)?d_vz(gidz-2,gidx):0.0;
  	sh_vx(lidz-2,lidx) = (gidz >= 2)?d_vx(gidz-2,gidx):0.0;
  }
	
	if(lidx < 4){
  	sh_vz(lidz,lidx-2) = (gidx >= 2)?d_vz(gidz,gidx-2):0.0;
  	sh_vx(lidz,lidx-2) = (gidx >= 2)?d_vx(gidz,gidx-2):0.0;
  }
 //  if(gidz < 2){
 //   	sh_vz(lidz-2,lidx) = 0.0;
 //  	sh_vx(lidz-2,lidx) = 0.0; 	
 //  }else if(lidz < 4){
 ...
 //  }
	...
	// if(gidx < 2){
	// 	sh_vz(lidz,lidx-2) = 0.0;
 //  	sh_vx(lidz,lidx-2) = 0.0;
	// }else	if(lidx < 4){
