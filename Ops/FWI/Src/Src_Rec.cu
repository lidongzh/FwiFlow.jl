...
Src_Rec::Src_Rec(Parameter &para, string survey_fname, string dir) {
	...
	for (int i = 0; i < nShots; i++) {
		...
		cout << "	Src_fname = " << dir + json_src_rec[thisShot]["Src_fname"].GetString() << endl;
		fileBinLoad(h_source, nSteps, dir + json_src_rec[thisShot]["Src_fname"].GetString());
		//
