...
supported_types = ["float", "double", "int", "bool"]
used_names = set([])
...
def typeerror(tp):
    if tp not in supported_types:
        print("ERROR: type {} not supported".format(tp))
        exit(0)
    return tp
...
def BackwardInputOutput():
    s = ""
    for i in range(len(outputs)):
        s += "{}.Input(\"grad_{} : {}\")\n".format("" if i==0 else "  ", outputs[i][1], outputs[i][0])
    for i in range(len(inputs)):
...
def BackwardTensor():
    ...
    for i in range(len(outputs)):
        item = outputs[i]
        s += BackwardTensor_T2.substitute({"name": item[1], "id":i})
    for i in range(len(inputs)):
        item = inputs[i]
...
def BackwardTensorShape():
    ...
    for i in range(len(outputs)):
        item = outputs[i]
        s += BackwardTensorShape_T2.substitute({"name": item[1]})
    return s
...
def BackwardCheckShape():
    ...
    for i in range(len(outputs)):
        item = outputs[i]
        s += BackwardCheckShape_T2.substitute({"name": item[1], "dims": item[2]})
    return s


BackwardOutputShape_T = Template("""
    TensorShape grad_${name}_shape(${dims});""")
...
def BackwardGetData():
    ...
    for i in range(len(outputs)):
        item = outputs[i]
        s += BackwardGetData_Input_T2.substitute({"name": item[1], "tp": item[0]})
    for i in range(len(inputs)):
        item = inputs[i]
...
with open("custom_op.template","r") as fp:
    cnt = fp.read()
    s = Template(cnt)
    cpp = s.substitute(d)
...
d = {"include": tf.sysconfig.get_compile_flags()[0][2:],
    ...
    "OperatorName": op}
with open("CMakeLists.template","r") as fp:
    cnt = fp.read()
    s = Template(cnt)
...
d = {"operator_name": convert(op),
    ...
    "OperatorName": op,
    "GetAttr": GetAttr()}
with open("gradtest.template","r") as fp:
    cnt = fp.read()
    s = Template(cnt)
    jl = s.substitute(d)
