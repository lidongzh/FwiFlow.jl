...
void forward(double *out, const double *perm, const double *mobi, \
	...
	for (int i = 0; i < nz; i++) {
		for (int j = 0; j < nx; j++) {
			perm_r = 0.0; perm_l = 0.0; perm_d = 0.0; perm_u = 0.0;
			mobi_r = 0.0; mobi_l = 0.0; mobi_d = 0.0; mobi_u = 0.0;
			...
			out(i, j) = perm_d * mobi_d * F_d - perm_u * mobi_u * F_u + perm_r * mobi_r * F_r - perm_l * mobi_l * F_l;
...
void backward(const double *grad_out, const double *perm, const double *mobi, \
              const double *func, double h, double rhograv, double *grad_perm, double *grad_mobi, \
              double *grad_func, int nz, int nx) {

	intialArray(grad_perm, nz * nx, 0.0);
	intialArray(grad_mobi, nz * nx, 0.0);
	...
	for (int i = 0; i < nz; i++) {
		for (int j = 0; j < nx; j++) {
			...
			if (j + 1 <= nx - 1) {
				...
				grad_perm(i, j) += grad_out(i, j) * mobi_r * F_r * grad_harAve(perm(i, j), perm(i, j + 1), true) / h2;
				grad_perm(i, j + 1) += grad_out(i, j) * mobi_r * F_r * grad_harAve(perm(i, j), perm(i, j + 1), false) / h2;
			...
			if (j - 1 >= 0) {
				...
				grad_perm(i, j) += -grad_out(i, j) * mobi_l * F_l * grad_harAve(perm(i, j), perm(i, j - 1), true) / h2;
				grad_perm(i, j - 1) += -grad_out(i, j) * mobi_l * F_l * grad_harAve(perm(i, j), perm(i, j - 1), false) / h2;
			...
			if (i + 1 <= nz - 1) {
				...
				grad_perm(i, j) += grad_out(i, j) * mobi_d * F_d * grad_harAve(perm(i, j), perm(i + 1, j), true) / h2;
				grad_perm(i + 1, j) += grad_out(i, j) * mobi_d * F_d * grad_harAve(perm(i, j), perm(i + 1, j), false) / h2;
			...
			if (i - 1 >= 0) {
				...
				grad_perm(i, j) += -grad_out(i, j) * mobi_u * F_u * grad_harAve(perm(i, j), perm(i - 1, j), true) / h2;
				grad_perm(i - 1, j) += -grad_out(i, j) * mobi_u * F_u * grad_harAve(perm(i, j), perm(i - 1, j), false) / h2;
