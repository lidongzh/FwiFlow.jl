...
void forward(double *p, const double *coef, const double *g, double h, int nz, int nx) {
	...
	Eigen::SparseMatrix<double, Eigen::RowMajor> Amat(nz * nx - 1, nz * nx - 1);
	Eigen::VectorXd rhs(nz * nx - 1), pvec(nz * nx - 1);
	int idRow = 0;
	double a = 1.0, b = 1.0, c = 1.0, d = 1.0;
	double h2 = h * h;
	for (int i = 0; i < nz; i++) {
		for (int j = 0; j < nx; j++) {
			idRow = i * nx + j - 1; // row-major
			if (i == 0 && j >= 1 && j <= nx - 2) {
				// upper BD
				...
				// upper-left corner
				// a = (coef(i, j) + coef(i, j + 1)) / 2.0;
				// b = 1.5 * coef(i, j) - coef(i, j + 1);
				// c = (coef(i, j) + coef(i + 1, j)) / 2.0;
				// d = 1.5 * coef(i, j) - coef(i + 1, j);
