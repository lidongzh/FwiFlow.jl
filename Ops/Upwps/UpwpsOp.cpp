...
class UpwpsOpOp : public OpKernel {
  ...
  public:
    ...
    void Compute(OpKernelContext *context) override {
        ...
        // TODO:
        forward(pres_tensor, permi_tensor, mobi_tensor, src_tensor, funcref_tensor, *h_tensor, *rhograv_tensor, *index_tensor, nz, nx);
...
class UpwpsOpGradOp : public OpKernel {
  ...
  public:
    ...
    void Compute(OpKernelContext *context) override {
        ...
        OP_REQUIRES_OK(context, context->allocate_output(0, grad_permi_shape, &grad_permi));
        Tensor *grad_mobi = NULL;
        OP_REQUIRES_OK(context, context->allocate_output(1, grad_mobi_shape, &grad_mobi));
        Tensor *grad_src = NULL;
        OP_REQUIRES_OK(context, context->allocate_output(2, grad_src_shape, &grad_src));
        Tensor *grad_funcref = NULL;
        OP_REQUIRES_OK(context, context->allocate_output(3, grad_funcref_shape, &grad_funcref));
        Tensor *grad_h = NULL;
        OP_REQUIRES_OK(context, context->allocate_output(4, grad_h_shape, &grad_h));
        Tensor *grad_rhograv = NULL;
        OP_REQUIRES_OK(context, context->allocate_output(5, grad_rhograv_shape, &grad_rhograv));
        Tensor *grad_index = NULL;
        OP_REQUIRES_OK(context, context->allocate_output(6, grad_index_shape, &grad_index));
        ...
        backward(grad_pres_tensor, pres_tensor, permi_tensor, mobi_tensor, \
                 src_tensor, funcref_tensor, *h_tensor, *rhograv_tensor, *index_tensor, nz, nx, \
                 grad_permi_tensor, grad_mobi_tensor, grad_src_tensor);
