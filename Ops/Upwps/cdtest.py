...
def upwps_op(permi,mobi,src,funcref,h,rhograv,index):
    ...
    return pres, grad
# Upwps_module = tf.load_op_library('build/libUpwpsOp.so')
...
ones_cons = tf.constant(np.ones((nz,nx)), dtype=tf.float64)
# g_np = g_np - np.mean(g_np)
g = tf.constant(g_np, dtype=tf.float64)
tf_h = tf.constant(h, dtype=tf.float64)

p = upwps_op(coef, ones_cons, g, ones_cons, tf_h,tf.constant(G*rho, dtype=tf.float64), tf.constant(0, dtype=tf.int64))
