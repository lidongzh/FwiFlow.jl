...
*/
void compRes(Eigen::VectorXd &resEg, Eigen::MatrixXd &sEg, const double* dporodt, const double *s0,
...
double linesearch(Eigen::MatrixXd &sEg, const Eigen::MatrixXd &delta_sEg_mat,
  ...
  compRes(resEg, sEg, dporodt, s0, pt, permi, poro, qw, qo, dt, h, nz, nx);
  double res_norm = resEg.lpNorm<Eigen::Infinity>();
  // double res_norm = resEg.norm();

  sEg_update = sEg + alpha * delta_sEg_mat;
  compRes(resEg, sEg_update, dporodt, s0,  pt, permi, poro, qw, qo, dt, h, nz, nx);
...
forward function
*/
void forward(double *sat, const double* dporodt, const double *s0, const double *pt,
  ...
  while (abs(res_norm - res_norm_old) > NEWTON_TOL * res_norm_old) {
    ...
    compRes(resEg, sEg, dporodt, s0, pt, permi, poro, qw, qo, dt, h, nz, nx);
    assembleJ(Jac, sEg, dporodt, pt, permi, poro, qo, dt, h, nz, nx);
    sEg_old = sEg;
// ================ AMG ====================
#ifdef DEBUG
    ...
    res_norm = linesearch(sEg_old, delta_sEg_mat, sEg, dporodt, s0,  pt, permi, poro, qw,
                          qo, dt, h, nz, nx);
#ifdef DEBUG
    printf("res_norm = %f\n", res_norm);
#endif
